...
def test_select_action(key, action_space: Box, agent: Agent):
    ...
    a1, _ = agent.select_action(key, obs, None, agent.params)
    ...
    # the same actions
    a2, _ = agent.select_action(key, obs, None, agent.params)
...
def test_select_action_different_keys(key, action_space: Box, agent: Agent):
    ...
    a1, _ = agent.select_action(key1, obs, None, agent.params)
    a2, _ = agent.select_action(key2, obs, None, agent.params)
