...
def create_env(
    ...
    if processed_x_target.ndim == 0:
        ...
        raise ValueError(
            f"Length of x_target ({len(processed_x_target)}) must match max_steps ({config.max_steps})."
        )
...
@partial(jax.jit, static_argnames=['config'])
def jit_reset(
    key: chex.PRNGKey, params: EnvParams, config: EnvConfig
